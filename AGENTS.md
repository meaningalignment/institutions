...
- **`/researchers` — Community page.** Three groups (see `getCommunity`): **Scouts & Advisors** (the `advisors` table, shown with their `advises_about`), **Community members** (anyone with a signal/zoom/workshop `researcher_involvements` row), and **Friends** (`commitment` in `committed`/`warm`, not already grouped). Cards + `/researchers/:handle` profiles are styled with Tailwind (`app/components/ResearcherCard.tsx`). Researcher `photo_url`s are site-relative and prefixed with `https://players.meaningalignment.org`. **Currently unlinked** from the grids (re-add the link in `app/components/Controls.tsx` when launching).
- **`/admin`.** Unlinked internal dashboard to configure, per researcher: scout/advisor status + `advises_about`, involvement membership, and associated papers (`canonical_works`). It writes to the DB via route `action`s. **No auth yet** — gate it (e.g. an `ADMIN_TOKEN` check) before exposing it on the live domain.
