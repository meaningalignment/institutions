...
`/resources` lists the research network's work by field, from the `canonical_works` table (see "Works in the DB" below). A left rail (a select on mobile) picks a field via `?field=<research-field id>`; with a field selected, the header becomes that field: the field name as H1, a one-line scope, its one corresponding researcher, a "Relevant for" link to the AGI grid highlighting the institutions that draw on it, and a forwarding email alias (copy and contacts in `FIELD_INFO` in `app/routes/resources.tsx`). Author and curator names link only to the person's X account when the roster has a `handle`, and are plain text otherwise; never to Google Scholar or `/researchers` profiles (`getResearcherLinks()`; names are matched on first + last name via `researcherKey`, so middle initials and hyphenation don't matter). People not in the roster stay plain text. Below come three collapsible sections, driven by each work's `section`: **Selected papers** (title, byline, one-line summary), **Work in the field** (title and byline), and **Background work** (older classics the field builds on, oldest first). Bylines end with venue and publication kind ("2022 · PNAS · Peer-reviewed"). "Elsewhere" (external bibliographies, hardcoded in the route) shows only under All fields. Grid cells and fields are joined by one map, `getCellFieldMap()` in `app/lib/researchers.server.ts`: each cell's top three fields by number of listed works naming it in `cells` (cached per instance for five minutes). It drives the "Further reading" line on AGI cell pages, the "Relevant for" count on `/resources`, and the AGI grid's `?field=<id>` highlight (matching cells outlined, the rest dimmed, with a note linking back) and its final **Research fields** row (each column's top three fields, linking to `/resources?field=`). All three degrade gracefully if the DB is unavailable.

### Works in the DB

All works live in `canonical_works` (named for its original role as the scouts' picks); the schema is owned by `../ecosystem/prisma/schema.prisma`. Columns beyond `title`/`url`: `authors[]` (the work's own order), `year`, `venue`, `kind` (enum `WorkKind`), `summary` (shown for selected works), `section` (enum `ResourceSection`: `selected` | `field` | `background`; **NULL hides a work** without deleting it), `fields[]` (explicit `RESEARCH_FIELDS` ids — a work appears only in the fields listed here), and `cells[]` (grid cells it bears on). Roster members are linked through `researcher_canonical_works`, which also drives researcher profiles (selected vs. other work). To see exactly what the page shows, browse the `resources_listing` view in the Neon console: one row per (field, work) in page order, hidden works last. Recreate it with `bun run db:migrate:resources-view`.

Schema changes go through `bun run db:push` (`scripts/db-push.sh`), which runs Prisma from `../ecosystem`, prints the SQL diff, and asks before applying. The database is shared with the ecosystem app, and tables this app creates (`advisors`, `institutions_admin_login_codes`, `institutions_project_updates`) must also be declared in that Prisma schema, or a push would drop them.
